...
// Valid UUID format placeholder - used only when env.json is unavailable
...
// Preload config immediately - call this at app startup
...
export async function loadConfig(): Promise<Config> {
  // If preload was called, use the existing promise
  if (configPromise) {
...
async function loadConfigInternal(): Promise<Config> {
  if (configCache) {
    return configCache;
  }
